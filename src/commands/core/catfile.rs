...
fn read_vox_object(hash: &str) -> Result<Vec<u8>> {
    ...
    decoder
        ...
        .with_context(|| format!("Failed to read object data"))?;
...
fn parse_object_header(data: &[u8]) -> Result<(Object, &[u8])> {
    let header_end = data
        .iter()
        .position(|&b| b == b'\0')
        .with_context(|| format!("Failed to find header end"))?;
...
fn display_tree_content(data: &[u8]) -> Result<()> {
    let mut pos = 0;
    while pos < data.len() {
        let entry = parse_tree_entry(&data[pos..])
            .with_context(|| format!("Failed to parse tree entry"))?;
